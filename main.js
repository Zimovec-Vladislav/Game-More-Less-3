...
function superGame(min, max, attemptsNumber, randomNumber, playerName) {
    alert(`Привет ${playerName}! Я загадал число от ${min} до ${max}. У тебя ${attemptsNumber} попыток. Вводи только числа, иначе ты меня сломаешь!`)

    let playerAnswers = []

    for (let attempt = 1; attempt <= attemptsNumber; attempt++){
        let playerAnswer = parseFloat(prompt('Введи число', ''))

        while(isNaN(playerAnswer)){
            playerAnswer = parseFloat(prompt('Введи число', ''))
        }
        ...
        if (playerAnswer > randomNumber) {
            ...
            alert(`Моё число больше! У тебя осталось ${attemptsNumber - attempt}`)
            playerAnswers.push(playerAnswer)
        } else {
            alert(`Поздравляю! Ты разгадал мою зловещую тайну за ${attempt} попыток`)
            return
        }
    }
